...
//----------------------------------------------------------
void ofTextureAdv::setPoints(ofPoint outputPts[pointNum]){
...
//----------------------------------------------------------
void ofTextureAdv::setPoints(ofPoint inputPts[pointNum], ofPoint outputPts[pointNum]){
	for (int i = 0; i < pointNum; i++){
	    quad[i].set(outputPts[i].x, outputPts[i].y, 0);
...
void ofTextureAdv::updatePoints(){
	...
	for(int y = 0; y < gridSizeY; y++){
		for(int x = 0; x < gridSizeX; x++){
			...
			coor[y*gridSizeX + x].set( (tt * texTAdj), texData.bFlipTexture ? texUAdj - (uu * texUAdj) : (uu * texUAdj), 0);
		}
	}
